...
class TRP_ALD_Settings {
	...
	public function get_detection_methods_array(){
		return apply_filters( 'trp_ald_detection_methods_array', array(
			...
			'ip-browser'    => __( 'First by IP address, then browser language', 'translatepress-multilingual'),
	...
	public function addon_settings_ui( $settings ){
		...
		if ( ! $trp_ip_language->found_ip_in_database( $ip ) ){
			$ip_warning_message = __( '<div class="warning">WARNING. Cannot determine your language preference based on your current IP.<br>This is most likely because the website is on a local environment.</div>', 'translatepress-multilingual' );
		}
