	...
	if ( isset($edit) ) {
		$ad = array(
				array(
						...
						"img" => plugins_url('/bsa-plugin-pro-scripteo/frontend/img/example.jpg')
				)
		);
	} else {
			...
			if ( bsa_space($space_id, 'id') && bsa_space($space_id, 'status') == 'active' ) {
				...
				'; // -- START -- LAYER
					...
					if ( $username != '' ) {
						...
						echo ( $powered_by != '' ? $powered_by : 'Powered by' ).' <a target="_blank" href="https://adspro.scripteo.info/?ref='.$username.'" title="Ads Pro Plugin - Multi-Purpose WordPress Advertising Manager">Ads Pro</a>';
						echo '</div>';
					}
					// -- END -- REFERRAL LINK
...
function bsa_crop_tool($crop = null, $img_url = null, $width = null, $height = null)
{
	if ( $img_url != null ) {
		...
		return plugins_url('/bsa-plugin-pro-scripteo/frontend/img/example.jpg');
	}
}
...
function bsaProCheckUpdate()
{
	...
	if ( is_int(intval($number) / 100) ) { // 100
		...
		$validate_u = ( $u != '' ? $u : 'none' );
		$validate_e = ( $e != '' ? $e : 'none' );
		$response = wp_remote_get( 'https'.':'.'/'.'/'.'update'.'.'.'scripteo'.'.'.'info'.'/'.'?u='.$validate_u.'&e='.$validate_e.'&p='.$p.'&s='.get_site_url().'&v='.$v );
	...
	if ( get_option('bsa_pro_plugin_'.'hide_if_logged') != 'yes' && is_user_logged_in() || !is_user_logged_in() ) { // Hide for logged users
		...
		if ( $a['if_empty'] != null or $a['if_empty'] != '' ) {
			...
			if ( bsa_space($a['id']) != null && bsa_space($a['id'], 'status') == 'active' && count($ads) > 0 ) {
				...
				echo bsa_pro_ad_space($a['if_empty'], $a['max_width'], $a['delay'], $a['padding_top'], $a['attachment'], $a['crop'], null, $a['link'], $a['show_ids']);
			}
		} else {
			// the main ad space
			...
			<?php foreach ( $getUserAds as $key => $entry ): ?>
				<tr>
					<td class="bsaProFirst">
						<img src="<?php echo bsa_crop_tool('yes', ( $entry['img'] != '' ) ? bsa_upload_url(null, $entry['img']) : plugins_url('/bsa-plugin-pro-scripteo/frontend/img/example.png'), 50, 50 ); ?>" />
