jQuery( document ).ready( function ( $ ) {
    ...
    /**
     * wp_localize_script object: ulwmImageActionArgs
     *
     * Params:
     *
     * _nonce
	...
	init: function () {
		...
		if ( !ulwmImageActionArgs.backup_image && action === 'removewatermark' ) {
		    return;
		}
		...
		if ( 'applywatermark' == action || 'removewatermark' == action ) {
			...
			$( '.ulwm-notice' ).slideUp( 'fast', function () {
			    $( this ).remove();
			} );
			...
			watermarkImageActions.notice( 'ulwm-notice error', ulwmImageActionArgs.__running, false );
		    }
	    ...
	    $( document ).on( 'click', '#ultimate_watermark_buttons a.ulwm-watermark-action', function ( e ) {
		// Get the selected bulk action
		...
		if ( 'applywatermark' == action || 'removewatermark' == action && !isNaN( id ) ) {
			...
			// script is running, can't run two at the same time
			watermarkImageActions.notice( 'ulwm-notice error', ulwmMediaModal.__running, false );
		    }
		}
	    } );

	    // Since these are added later we'll need to enable dismissing again
	    $( document ).on( 'click', '.ulwm-notice.is-dismissible .notice-dismiss', function () {
		$( this ).parents( '.ulwm-notice' ).slideUp( 'fast', function () {
		    $( this ).remove();
		} );
	    } );
	...
	post_loop: function () {
		...
		if ( !isNaN( id ) ) {
		    ...
		    // post data
		    data = {
			'_ulwm_nonce': ulwmImageActionArgs._nonce,
			'action': 'ulwm_watermark_bulk_action',
			'ulwm-action': watermarkImageActions.action,
			'attachment_id': id
		    };
			...
			$( '.ulwm-overlay' ).first().each( function () {
			    $( this ).fadeOut( 'fast', function () {
				$( this ).remove();

				if ( response.data === 'watermarked' ) {
				    $( '#ultimate_watermark_buttons .value' ).append( '<span class="dashicons dashicons-yes" style="font-size: 24px;float: none;min-width: 28px;padding: 0;margin: 0; display: none;"></span>' );
				    $( '#ultimate_watermark_buttons .value .dashicons' ).fadeIn( 'fast' );
				} else if ( response.data === 'watermarkremoved' ) {
				    $( '#ultimate_watermark_buttons .value' ).append( '<span class="dashicons dashicons-yes" style="font-size: 24px;float: none;min-width: 28px;padding: 0;margin: 0; display: none;"></span>' );
				    $( '#ultimate_watermark_buttons .value .dashicons' ).fadeIn( 'fast' );
				}

				$( '#ultimate_watermark_buttons .value .dashicons' ).delay( 1500 ).fadeOut( 'fast', function () {
				    $( this ).remove();
				} );
			    } );
			} );
	...
	result: function ( response, id ) {
		...
		switch ( response.data ) {
			...
			type = 'ulwm-notice updated ulwm-watermarked';
			...
			if ( 1 < watermarkImageActions.successCount ) {
			    //yes
			    message = ulwmImageActionArgs.__applied_multi.replace( '%s', watermarkImageActions.successCount );
			} else {
			    //no
			    message = ulwmImageActionArgs.__applied_one;
			}
			...
			// The css classes for the notice
			type = 'ulwm-notice updated ulwm-watermarkremoved';
			// another successful update
			watermarkImageActions.successCount += 1;
			// did we have more success updates?
			if ( 1 < watermarkImageActions.successCount ) {
			    //yes
			    message = ulwmImageActionArgs.__removed_multi.replace( '%s', watermarkImageActions.successCount );
			} else {
			    //no
			    message = ulwmImageActionArgs.__removed_one;
			}
			// update the row feedback
			watermarkImageActions.row_image_feedback( 'success', id );
			// reload the image
		...
		// No success...
		watermarkImageActions.notice( 'ulwm-notice error', response.data, false );
		// update the row feedback
		watermarkImageActions.row_image_feedback( 'error', id );
	    }
	...
	row_image_feedback: function ( type, id ) {
		...
		case 'media-modal':
		    container_selector = '.media-modal #ultimate_watermark_buttons[data-id="' + id + '"] .value';
		    css = {
			'float': 'none'
		    };
		    cssinner = {
	    ...
	    if ( !$( container_selector + ' .ulwm-overlay' ).length ) {
		$( container_selector ).append( '<span class="ulwm-overlay"><span class="ulwm-overlay-inner"></span></span>' );
	    }

	    // Overwrite with new data
	    $( container_selector + ' .ulwm-overlay' ).css( css );
	    $( container_selector + ' .ulwm-overlay .ulwm-overlay-inner' ).css( cssinner );
	    $( container_selector + ' .ulwm-overlay .ulwm-overlay-inner' ).html( '<span class="spinner is-active"></span>' );

	    if ( watermarkImageActions.action_location === 'media-modal' ) {
		$( container_selector + ' .ulwm-overlay .ulwm-overlay-inner .spinner' ).css( { 'float': 'none', 'padding': 0, 'margin': '-4px 0 0 10px' } );
	    }
	},
	notice: function ( type, message, overwrite ) {
		...
		case 'upload-list':
		    ...
		    break;
		default:
		    prefix = '#ultimate_watermark_buttons';
		    break;
	    }
		...
		switch ( watermarkImageActions.response ) {
		    case 'watermarked':
			selector = '.ulwm-notice.ulwm-watermarked';
			break;
		    case 'watermarkremoved':
			selector = '.ulwm-notice.ulwm-watermarkremoved';
			break;
		    case 'skipped':
			selector = '.ulwm-notice.ulwm-skipped';
			break;
		}
		...
		if ( selector && $( '.wrap ' + selector + ' > p' ).length ) {
		    ...
		    $( prefix ).after( '<div class="' + type + '" style="display: none;"><p>' + message + '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' + ulwmImageActionArgs.__dismiss + '</span></button></div>' );
		    $( '.ulwm-notice' ).slideDown( 'fast' );
		}
	    } else {
		// create a new notice
		$( prefix ).after( '<div class="' + type + '" style="display: none;"><p>' + message + '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' + ulwmImageActionArgs.__dismiss + '</span></button></div>' );
    ...
    // We need that nonce!
    if ( typeof ulwmImageActionArgs._nonce != 'undefined' ) {
