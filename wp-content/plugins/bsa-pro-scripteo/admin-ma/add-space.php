...
<h2>
	<?php if ( isset($_GET['space_id']) ): ?>
		...
		<p><span class="dashicons dashicon-14 dashicons-arrow-left-alt"></span> <a href="<?php echo admin_url(); ?>admin.php?page=bsa-pro-sub-menu-ma-spaces<?php echo ((isset( $_GET['site_id'] )) ? '&site_id='.$_GET['site_id'] : null) ?><?php echo ((isset( $_GET['space_id'] )) ? '&space_id='.$_GET['space_id'] : null) ?>">back to <strong>spaces / ads list</strong></a></p>
	<?php else: ?>
...
<?php if ( isset($_GET['space_id']) && bsa_space($_GET['space_id'], 'id') != NULL && $sites != NULL || !isset($_GET['space_id']) && $sites != NULL ): ?>

	<form action="" method="post" enctype="multipart/form-data">
	<?php if ( isset($_GET['space_id']) ): ?>
		<input type="hidden" value="updateMaSpace" name="bsaProAction">
	<?php else: ?>
		<input type="hidden" value="addMaNewSpace" name="bsaProAction">
	<?php endif; ?>
	...
	<?php endif; ?>
	<tr>
		<th scope="row"><label for="bsa_pro_site_id">Choose Site</label></th>
		<td>
			<select id="bsa_pro_site_id" name="site_id">
				<?php foreach ($sites as $site) {
					?>
					<option value="<?php echo $site['id']; ?>" <?php selectedSpaceOpt('site_id', $site['id']); ?>> <?php echo $site['title'].', ID: '.$site['id'] ?></option>
				<?php
				} ?>
			</select>
		</td>
...
<?php else: ?>
	...
	<div class="updated settings-error" id="setting-error-settings_updated">
		<p><strong>Error!</strong> Site not exists! Go <a href="<?php echo admin_url(); ?>admin.php?page=bsa-pro-sub-menu-ma-new-site">here</a> to add new site.</p>
	</div>
