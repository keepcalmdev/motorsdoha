...
<?php if ( isset($_GET['space_id']) && bsa_space($_GET['space_id'], 'id') != NULL && $site_id != NULL || !isset($_GET['space_id']) && $site_id != NULL ): ?>
	...
	<?php if ( $get_spaces ): ?>
		...
		<?php $siteUrl = parse_url(bsa_site($site_id, 'url')); ?>
		<?php $domain = str_replace('/', '', str_replace('www.', '', str_replace('http://', '', str_replace('https://', '', $siteUrl['host'])))); ?>
...
secure="<?php echo hash('sha1', $space_id.$domain.'bsa_pro'); ?>"]'); ?&gt;</pre>
					...
					<?php if(get_option('bsa_pro_plugin_'.'agency_other_sites') == 'yes'): ?>
						<?php
							$heightCss = explode('--', bsa_space($space_id, 'template'));
							$heightStyle = (isset($heightCss[1]) && $heightCss[1] != '' ? 'height:'.$heightCss[1].'px;' : null);
						?>
						<strong>Display Ads via Iframe for Non-WordPress Sites</strong><br>
						<pre style="max-width:500px;white-space:normal;">&lt;iframe style="border:0;<?php echo $heightStyle ?>" width="100%" height="auto" scrolling="no" id="bsaIframe" onload="bsaIframe()"
...
&lt;/iframe&gt;&lt;script&gt;function bsaIframe() { var iFrameID = document.getElementById('bsaIframe'); if(iFrameID) { iFrameID.height = iFrameID.contentWindow.document.body.scrollHeight + 'px'; } }&lt;/script&gt;</pre><br>
		...
		<h3>Active Ads (<?php echo count($getActiveAds) ?>)
			<div style="margin:10px 0 20px;">order by
				...
				<a href="<?php echo admin_url(); ?>admin.php?page=bsa-pro-sub-menu-ma-spaces&site_id=<?php echo $_GET['site_id']; ?>&space_id=<?php echo $space_id; ?>&order_ads=priority" class="bsaOrderBy <?php echo get_order_ads($space_id, 'priority'); ?>"><img src="<?php echo plugins_url('/bsa-plugin-pro-scripteo/frontend/img/icon-drag-16.png'); ?>" /> priority</a>
				...
				foreach ($getActiveAds as $key => $entry) {
					...
					<tr class="<?php echo $alternate; ?>" id="<?php echo $entry['id']; ?>">
						<td class="bsaAdminImg">
							<img class="bsaAdminThumb" src="<?php echo ( $entry['img'] != '' ) ? bsa_upload_url(null, $entry['img']) : plugins_url('/bsa-plugin-pro-scripteo/frontend/img/example.png'); ?>">
						</td>
		...
		<table class="wp-list-table widefat bsaListTable">
			...
			if (count($getSites) > 0) {
				foreach ($getSites as $key => $entry) {
					...
					<tr class="<?php echo $alternate; ?>">
						<td class="bsaAdminImg">
							<img class="bsaAdminThumb" src="<?php echo ( $entry['thumb'] != '' ) ? bsa_upload_url(null, $entry['thumb']) : plugins_url('/bsa-plugin-pro-scripteo/frontend/img/example.png'); ?>">
						</td>
						<td class="post-title page-title column-title">
