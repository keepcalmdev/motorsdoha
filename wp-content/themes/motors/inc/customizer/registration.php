...
if(!function_exists('is_listing')) {
    function is_listing($only = array()) {
        if(count($only) > 0) {
            ...
            if(stm_is_listing() || stm_is_listing_two() || stm_is_listing_three() || stm_is_listing_four()) return true;
        }
...
if (!stm_is_boats()) {
    if (stm_is_motorcycle() or stm_is_rental() || stm_is_magazine() || is_listing(array('listing_two', 'listing_three'))) {
...
STM_Customizer::setSection('typography', array(
    ...
    'fields' => array(
        ...
        'typography_menu_font_family' => array(
            ...
            'type' => 'stm-font-family',
            'output' => '.header-menu li a, .listing-menu li a, .header-listing .listing-menu li a, .stm-navigation ul li a',
        ),
        ...
        'typography_menu_color' => array(
            ...
            'type' => 'color',
            'output' => array('color' => '.header-menu li a, .listing-menu li a, .header-listing .listing-menu li a'),
            'transport' => 'postMessage',
            'default' => '#232628'
        ),
...
if (stm_get_current_layout() == 'boats' || stm_get_current_layout() == 'car_dealer_two') {
    ...
    $headers = (stm_get_current_layout() == 'boats') ? array(
