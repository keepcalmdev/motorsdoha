...
class PriceForDatePeriod {
    ...
    public static function updateVariationPrice ($price, $product) { // update total cart
        ...
        foreach ($dates as $k => $date) {
            ...
            $request = "SELECT * FROM {$table_name}
                          ...
                          (starttime BETWEEN {$date} AND {$endDate} OR
                          endtime BETWEEN {$date} AND {$endDate} OR
                          (starttime <= {$date} AND endtime >= {$endDate})) ORDER BY id DESC LIMIT 1";

            $response = self::$wpdbObj->get_results($request);

            if ($k < $countDays) {
                $price = (!empty($price)) ? $price : 0;
                $newPrice += (!empty($response) && isset($response[0])) ? $response[0]->price : $price;
            }
        }
    ...
    public static function checkEntry ($startTime, $endTime, $varId) {
        ...
        $result = self::$wpdbObj->get_results("SELECT id FROM " . self::$wpdbObj->prefix ."rental_price_date WHERE `variation_id` = {$varId} AND `starttime` = {$startTime} AND `endtime` = {$endTime}");
