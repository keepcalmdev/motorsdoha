...
<div class="stm_add_car_form_1">
	...
	<?php if(!empty($taxonomy)): ?>
		...
		<style type="text/css">
			...
			.stm-form1-intro-unit .select2-selection__rendered[title="<?php esc_html_e('Select', 'motors'); ?> <?php printf(_x('%s', 'Add A Car Step 1 Slug Name', 'motors'), stm_get_name_by_slug($tax)); ?>"] {
				background-color: transparent !important;
				border: 1px solid rgba(255,255,255,0.5);
				color: #fff !important;
			}
			.stm-form1-intro-unit .select2-selection__rendered[title="<?php esc_html_e('Select', 'motors'); ?> <?php printf(_x('%s', 'Add A Car Step 1 Slug Name', 'motors'), stm_get_name_by_slug($tax)); ?>"] + .select2-selection__arrow b {
		...
		<?php if ( ! empty( $data ) ): ?>
			...
			<div class="stm-form-1-quarter">
				...
				<div class="stm-label">
					...
					<?php endif; ?>
					<?php printf(_x('%s', 'Add A Car Step 1 Taxonomy Label', 'motors'), $data_unit['single_name']); ?>
				</div>
			</div>
        <?php endif; ?>
		<?php endforeach; ?>
			...
			<style type="text/css">
				...
				.stm-form-1-end-unit .select2-selection__rendered[title="<?php echo esc_attr__('Select', 'motors'); ?> <?php printf(_x('%s', 'Add A Car Step 1 Taxonomy Label', 'motors'), $data_unit['single_name']); ?>"] {
					background-color: transparent !important;
					border: 1px solid rgba(255, 255, 255, 0.5);
					color: #888 !important;
				}
