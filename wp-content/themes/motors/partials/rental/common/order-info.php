    ...
    <div class="stm_rent_table">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td><?php echo (isset($car_rent['hours']) && !empty($car_rent['hours'])) ? sprintf(esc_html__('%s Days %s Hours', 'motors'), $car_rent['days'], $car_rent['hours']) : sprintf(esc_html__('%s Days', 'motors'), $car_rent['days']); ?></td>
                    <td>
                ...
                <tbody>
                    ...
                    <?php foreach($cart_items['options'] as $car_option): ?>
                        <tr>
                            <td>
                                <?php
                                    $quant = (!empty(get_post_meta($car_option['id'], '_car_option', true))) ? $car_option['quantity'] : $car_option['quantity']/$car_option['opt_days'];
                                    echo sprintf(esc_html__('%s x %1s %s %s day(s)', 'motors'), ((!empty($car_option['quantity']) && !empty($car_option['opt_days'])) ? $quant : 0), $car_option['name'], esc_html__("for", 'motors'), $car_option['opt_days']);
                                ?>
                            </td>
